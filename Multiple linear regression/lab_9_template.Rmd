...
homes$Status <- factor(homes$Status)
...
# a. AIC values for each model # absolute AIC value means nothing, it's only valuable when it's looked as a comparative tool
...
predict_graph <- ggplot(predict_df, aes(x = SqFt, y = fit.fit))+ geom_line(aes(color = City)) +
